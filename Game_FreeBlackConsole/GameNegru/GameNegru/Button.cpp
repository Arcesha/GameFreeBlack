    ...
    : text(text), clicked(false) // Конструктор класса Button, инициализация полей
{
    ...
    if (!imagePath.empty()) {
        ...
        SDL_Surface* surface = IMG_Load(imagePath.c_str()); // Загружаем изображение
        if (!surface) {
            std::cerr << "Не удалось загрузить изображение: " << imagePath << " SDL_Error: " << IMG_GetError() << std::endl;
        ...
        SDL_FreeSurface(surface); // Освобождаем поверхность после создания текстуры
        if (!texture) {
